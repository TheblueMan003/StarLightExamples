...
class DisplayEntity extends Entity{
    ...
    Interpolate the entity's rotation.
    """
    def lazy interpolateLeftRotationQuaternion(int duration, float x, float y, float z, float w){
        data.set({"start_interpolation":0, "interpolation_duration":duration,"transformation":{"left_rotation":[x f, y f, z f, w f]}})
    }

    """
    Interpolate the entity's rotation.
    """
    def lazy interpolateRightRotationQuaternion(int duration, float x, float y, float z, float w){
        data.set({"start_interpolation":0, "interpolation_duration":duration,"transformation":{"right_rotation":[x f, y f, z f, w f]}})
    }
